...
> Status: this document describes the current Office.js MVP. The approved Windows product architecture is defined in [technical-solution.md](technical-solution.md).
...
## Current Office.js MVP strategy

The web add-in deliberately uses ES5 syntax and the WordApi 1.1 surface. The approved Windows product uses a VSTO host with an embedded WebView2 editor to provide richer selection events, keyboard integration, local TeX access, and legacy OLE migration. The current Office.js adapter and the target Windows host must call the same parser and produce the same FormulaBridge metadata schema.
